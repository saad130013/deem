...
const App: React.FC = () => {
  ...
  const handleStartQuiz = async () => {
    ...
    try {
      const questions = await generateQuiz(lessonData);
      setQuizData(questions);
      setAppState(AppState.QUIZ_VIEW);
    } catch (err) {
      console.error(err);
      setError("حدث خطأ أثناء تحضير الأسئلة.");
      setAppState(AppState.LESSON_VIEW); // Go back to lesson view on failure
    }
  };
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      ...
      <nav className="bg-white shadow-sm sticky top-0 z-50 px-4 py-3 border-b-2 border-gray-100 no-print">
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          <div className="flex items-center gap-2 cursor-pointer" onClick={resetApp}>
            <div className="bg-kid-purple p-2 rounded-lg text-white">
               <GraduationCap size={28} />
            </div>
            <h1 className="text-2xl font-black text-gray-800 tracking-tight">
              {lessonData?.teacherName ? (
                <span className="text-kid-purple">{lessonData.teacherName}</span>
              ) : (
                <>المعلم <span className="text-kid-pink">الذكي</span></>
              )}
            </h1>
          </div>
          ...
          <div className="flex gap-4">
             {appState === AppState.IDLE && (
                <button 
                  onClick={handleOpenSavedLessons}
                  className="text-gray-600 font-bold hover:text-kid-purple flex items-center gap-1 text-sm md:text-base px-3 py-2 rounded-lg hover:bg-purple-50 transition-colors"
                >
                  <Book size={18} />
             ...
             {appState !== AppState.IDLE && (
                <button 
                  onClick={resetApp} 
                  className="text-gray-500 font-bold hover:text-kid-blue flex items-center gap-1 text-sm md:text-base"
                >
                  <ArrowRight size={18} />
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 relative z-10">
        ...
        {error && (
            <div className="max-w-2xl mx-auto mb-6 bg-red-100 border-2 border-red-200 text-red-700 p-4 rounded-xl text-center font-bold no-print">
                {error}
            </div>
        )}
          ...
          <div className="animate-fade-in-up">
            <header className="text-center mb-12">
              <h2 className="text-4xl md:text-5xl font-black text-gray-800 mb-4 leading-tight">
                حوّل أفكارك إلى <br/>
                <span className="text-kid-blue">قصص</span> و <span className="text-kid-pink">ألعاب</span> تعليمية
              </h2>
              <p className="text-xl text-gray-500 max-w-2xl mx-auto">
                مساعد المعلم الذكي: صمم دروساً تفاعلية تناسب أعمار الأطفال في ثوانٍ معدودة.
              </p>
            </header>
            <LessonForm 
        ...
        {appState === AppState.SAVED_LESSONS_LIST && (
            <SavedLessonsList 
      ...
      <footer className="text-center py-8 text-gray-400 text-sm no-print">
         © {new Date().getFullYear()} المعلم الذكي - تم التطوير بحب للتعليم
      </footer>
    </div>
  );
};
